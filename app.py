...
/* Remove top padding so it feels like a takeover */
.block-container {{
padding-top: 1.2rem !important;
max-width: 900px;
}}

/* Hide Streamlit chrome (best effort; can change across versions) */
...
box = st.empty()
for f in frames:
box.markdown(f"<div style='font-size:84px; text-align:center;'>{f}</div>", unsafe_allow_html=True)
time.sleep(0.22)
...
code_str = st.text_input(f"Enter the code ({len(SECRET_CODE)} numbers):", placeholder="2 7 1 9")
...
st.markdown("<div style='color:white; font-size:44px; font-weight:900; text-align:center; margin-top:40px;'>WRONG FORMAT</div>", unsafe_allow_html=True)
speak_js(str(e))
time.sleep(1.0)
st.session_state.screen = "normal"
st.rerun()
...
st.markdown("<div style='color:white; font-size:44px; font-weight:900; text-align:center; margin-top:20px;'>✅ UNLOCKED</div>", unsafe_allow_html=True)
chest_animation()
st.markdown(f"<div style='color:#d1fae5; font-size:22px; text-align:center; font-weight:700;'>{WIN_TEXT}</div>", unsafe_allow_html=True)
speak_js(WIN_TEXT)
else:
st.session_state.screen = "error"
set_fullscreen_bg("error")
st.markdown("<div style='color:white; font-size:48px; font-weight:900; text-align:center; margin-top:60px;'>WRONG PASSWORD</div>", unsafe_allow_html=True)
